...
// Use this instead of whisper_pcm_to_mel() if you want to provide your own log mel spectrogram. n_mel must be 80
...
// Make sure to call whisper_pcm_to_mel() or whisper_set_mel() first. offset can be used to specify the offset of the first frame in the spectrogram.
int whisper_encode(struct whisper_context * ctx, int offset, int n_threads);
int whisper_encode_with_state(struct whisper_context * ctx, struct whisper_state * state, int offset, int n_threads);

// Run the Whisper decoder to obtain the logits and probabilities for the next token.
// Make sure to call whisper_encode() first. tokens + n_tokens is the provided context for the decoder.
