...
class Audio extends Controller {
    ...
    public static function speech_to_text(App $object){
        try {
            ...
            if($role){
                //move to service Audio::speech_to_text
                ...
                while(true){
                    ...
                    if($repository_record !== null){
                        ...
                        if(property_exists($record, 'output')){
                            ...
                            $url = $object->config('controller.dir.data') . 'Commands' . $object->config('extension.json');
                            $data = $object->data_read($url);
                            $commands = $data->get('Commands');
                            $route_url = '';
                            foreach($commands as $key => $command){
                                $node = new Node($object);
                                $record = $node->record('System.Route', $node->role_system(), [
                                    'where' => [
                                        '\'name\' === \'' . $command->route . '\'',
                                    ],
                                    'memory' => true
                                ]);
                                if(array_key_exists('REQUEST_SCHEME', $_SERVER) && $_SERVER['REQUEST_SCHEME'] === 'https'){
                                    $host = $node->record('System.Host', $node->role_system(), [
                                        'where' => [
                                            '\'url.production\' partial \'' . $record['node']->host . '\'',
                                        ],
                                        'memory' => true
                                    ]);
                                    ...
                                    if(
                                        ...
                                        property_exists($host['node'], 'url') &&
                                        property_exists($host['node']->url, $environment) &&
                                        !empty($host['node']->url->{$environment}) &&
                                        property_exists($record['node'], 'path')
                                    ){
                                        $route_url = $host['node']->url->{$environment} ?? '';
                                        ddd($record['node']->path);
                                        $route_url .= $record['node']->path;
                                    }
                                }
                                foreach($commands as $command){
                                    foreach($command->command as $text_command){
                                        if(stristr($output, $text_command)){
                                            //return json object
                                            $output = (object) [
                                                'url' => $route_url,
                                                'name' => $command->route
                                            ];
                                            echo Core::object($output, Core::JSON);
                                            exit(0);
                                        }
                                    }
                                }
     ...
     /**
        ...
        public static function speech_to_text_task(App $object): array | Exception
        {
            try {
                ...
                if($role){
                    //move to service Audio::speech_to_text_task
